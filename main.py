...
@app.get("/api/contacts/{contacts_id}",response_model=_schemas.Contact)
async def get_contact(contact_id:int,db:_orm.Session = _fastapi.Depends(_services.get_db)
     ...
     if contact is None :  
        raise _fastapi.HTTPException(status_code=404,detail="Contacto no encontrado")

     return contact

@app.delete("/api/contacts/{contacts_id}")
